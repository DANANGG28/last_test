...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div class="px-6 py-5 border-b border-gray-100 bg-gray-50/50">
            ...
            <p class="text-sm text-gray-500 mt-0.5">Lengkapi formulir di bawah ini untuk mendaftarkan produk baru ke katalog.</p>
        </div>

        <form method="POST" action="{{ route('products.store') }}" class="p-6 sm:p-8 space-y-6">
            @csrf
            ...
            <div>
                ...
                <textarea name="description"
                          ...
                          class="w-full px-4 py-2.5 text-sm rounded-lg border @error('description') border-red-300 ring-1 ring-red-500 bg-red-50/30 @else border-gray-300 @enderror focus:ring-2 focus:ring-blue-500 focus:border-blue-500">{{ old('description') }}</textarea>
                @error('description')
                    <p class="text-xs text-red-600 mt-1 font-medium">{{ $message }}</p>
                @enderror
            </div>
